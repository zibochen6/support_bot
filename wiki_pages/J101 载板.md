...
我们可以使用 **NVIDIA SDK Manager** Linux 软件，或者直接通过 Linux 命令行使用 **NVIDIA Linux Driver Package** 将 JetPack OS 刷写到 reComputer J1010。对于 NVIDIA Jetson 的初学者，我们强烈推荐使用 NVIDIA SDK Manager。
...
**步骤 3.** 使用 Type-C 线缆为 reComputer 供电，连接到 reComputer 左侧，并使用另一根线缆连接到 reComputer 右侧的 Linux 主机 PC。

![](https://files.seeedstudio.com/wiki/reComputer_flash_system/reComputerJ1010_J101_flash_Jetpack.png)

!!!Attention
在接线之前，请注意电源连接器和数据连接器，请不要将数据连接器直接连接到直流电源。
...
登录后，您将进入安装的第一个屏幕。由于我们已经连接了 reComputer J1010，会弹出一个窗口让您选择硬件设备。

reComputer J1010 配备了 **NVIDIA Jetson Nano 4GB 模块**，所以我们可以选择第一个。

![](https://files.seeedstudio.com/wiki/reComputer-Jetson-Nano/6.jpg)

在第一个屏幕中有更多选项供您选择：

* 产品类别面板中的 **Jetson** 需要被选中。
...
最近，NVIDIA 更新了 DRAM 型号，Seeed 目前已发布了与此模块更新兼容的新固件。
...
导致各种安装错误的原因很多。以下是常见安装问题的检查清单，可能有助于您从损坏的安装中恢复。
...
3. 重试安装还可以通过其他两种方式进行：
   ...
   a. 从**使用 SDK Manager 刷写到 eMMC -- 步骤 3**，使用修复/卸载按钮进入管理 NVIDIA SDK 页面。如果需要，展开状态为"损坏"的 SDK，然后点击相关部分（主机或目标）的修复。
