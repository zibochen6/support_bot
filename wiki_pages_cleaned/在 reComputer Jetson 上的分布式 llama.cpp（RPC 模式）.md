...
[TABLE COMPRESSED]
Columns: 设备 RAM 角色 IP | 机器 A 64 GB 客户端 + 本地服务器 192.168.100.2 | 机器 B 32 GB 远程服务器 192.168.100.1
...
[TABLE COMPRESSED]
Columns: 问题 解决方案 | rpc-server 启动失败 检查端口是否被占用或防火墙是否阻止了 50052/tcp | 推理速度较慢 模型太小，网络延迟 > 计算收益；尝试更大的模型或 Unix-socket 模式 | 内存不足错误 减少 `-ngl` 值以将更少的层卸载到 GPU，或将一些层保留在 CPU 上
