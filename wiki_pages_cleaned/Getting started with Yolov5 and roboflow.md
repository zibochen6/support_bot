...
* **Step 17.** Replace **MODEL\_KEY** with the **API key** that we copied before
...
Here the accuracy `[email protected]` is about 90% and 99.4% for leaf and flower respectively, while the total accuracy `[email protected]` is about 94.7%.
...
Here the accuracy `[email protected]` is about 91.6% and 99.4% for leaf and flower respectively, while the total accuracy `[email protected]` is about 95.5%.
...
Here the accuracy `[email protected]` is about 90.6% and 99.4% for leaf and flower respectively, while the total accuracy `[email protected]` is about 95%.
...
* **Step 11.** Copy **gen\_wts.py** from **tensorrtx/yolov5** into **yolov5** directory
```
...
```
vi yololayer.h
```* **Step 15.** Change **CLASS\_NUM** to the number of classes your model is trained. In our example, it is 2
```
CLASS_NUM = 2
...
```* **Step 11.** Copy **gen\_wts\_yoloV5.py** from **DeepStream-Yolo/utils** into **yolov5** directory
```
...
```* **Step 16.** Edit the **config\_infer\_primary\_yoloV5.txt** file according to your model
```
[property]  
...  
...
...
```* **Step 17.** Edit the **deepstream\_app\_config** file
```
...  
[primary-gie]  
...  
...
```* **Step 8.** Update the **config\_infer\_primary\_yoloV5.txt** file
...
[TABLE COMPRESSED]
Columns: Model Name Precision Inference Size Inference Time (ms) FPS | YOLOv5s FP32 320x320 16.66 60 | FP32 640x640 33.33 30 | INT8 640x640 16.66 60 | YOLOv5n FP32 640x640 16.66 60
...
[TABLE COMPRESSED]
Columns: Dataset Number of training samples Training time on Local PC (GTX 1660 Super) Training time on Google Colab (NVIDIA Tesla K80) | Custom 542 2.2 hours 1.3 hours | 240 1 hour 42 minutes | Pascal VOC 2012 17112 83 hours 15 hours | Microsoft COCO 2017 121408 750 hours 125 hours
